function caponBeamforming()

    % ------------------------------------------------
    %        GENERALISED CAPON BEAMFORMING DEMO
    ...
    circularSourceAngles = [-180 : 1 : -150];
    x_range = [-10 10];
    y_range = [-10 10];
...
%     
    ...
    %                 theta = atan(currentDistanceVector(2) / currentDistanceVector(1));
                ...
                outputPower(y_pos_index, x_pos_index) = 1 / ( ctranspose(vandermonde) * inv(covarianceMatrix) * vandermonde );
            end
        end
    ...
    fileNumber = 1;
    filenameToSave = sprintf('capon_beamformed_data_%d.mat', fileNumber);
